...
| POST   | `/auth/signup`          | body: `SignupDTO` `{name, email, phone, password}` | Creates user (default BASIC `AccountLevel`, `ACC-<uuid>` account number), returns `AuthenticationResponse` (201) + refresh cookie; already-used email → 409 "This email has already been used." | Public |
...
| GET    | `/savings-passbooks/search` | query: `principalAmount`, `savingsPassbookName`, `depositTerm`, `interestRate`, `maturityDate`, `withdrawalDate`, `estimatedMaturityProceeds` (all optional) + `page` (default 0), `size` (default 10) | Searches current user's passbooks via JPA `Specification` (`spec/SavingsPassbookSpecifications`). Filters are AND-combined; invalid value → 400. `savingsPassbookName` is a case-insensitive contains match on the plain value (no operators). The other six take optional `>` `>=` `<` `<=` prefixes (bare value = equality); date filters take ISO datetime or date-only (day-inclusive: `>` skips the whole day, `<=` runs through 23:59:59.999999999, bare date matches the whole day). Paged, sorted by `maturityDate` DESC | JWT  |
...
- Paged list endpoints (the owner-scoped GETs above) take `page`/`size` query params and return `PagedResponseDTO<T>` `{content, page, size, totalElements, totalPages, first, last}`. Sorts are fixed server-side; invalid paging (e.g. negative page) fails `PageRequest.of` → `IllegalArgumentException` → 400 via the catch-all handler.
