...
/**
 * Angular `date` pipe's 'mediumTime' equivalent (the pipe's default locale is
 * en-US) — used for the notification bell timestamps. Pinned to 'en-US' so
 * output does not drift with the host locale.
 */
export function formatMediumTime(isoString: string): string {
