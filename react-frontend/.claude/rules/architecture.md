...
src/
  ...
  app/
    ...
    hooks/                     use-page-title, use-auto-hide-scrollbar, use-mount-once
...
- **Initial page loads** go through `hooks/use-mount-once.ts` (`useMountOnce` — the Angular ngOnInit): a bare mount `useEffect` that fetches would double-fire every API call under dev StrictMode.
