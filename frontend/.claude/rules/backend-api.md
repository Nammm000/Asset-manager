...
The Spring Boot backend runs on port 8082 (`environment.apiUrl`, stateless JWT bearer auth). Every frontend service in `src/app/service/` maps 1:1 to a resource below; DTO interfaces in `src/app/model/` mirror the backend and document per-field quirks in doc comments.
...
- **Assets** (all user-scoped, JWT required): `/savings-passbooks`, `/land-assets`, `/other-assets` — paged `GET`, `GET /{id}`, `POST`, `PUT /{id}`, `DELETE /{id}` → `MessageResponse`. `/cash-assets` — same minus PUT. All four also take `DELETE /bulk` body `{ids: number[]}` → `MessageResponse` (all-or-nothing: 404 any id missing, 403 any not owned; deleting passbooks/cash wallets cascades their deposits/balances). `/cash-balances` — paged GET, `GET /{id}`, `POST` create, `POST /{id}` adjust (signed amount, negative subtracts), `PUT /{id}` `{amount}`, `DELETE /{id}`. `/additional-deposits` — `POST` only (email/phone must match account records) → updated `SavingsPassbook`.
...
- **Notifications** (`NotificationService`, WebSocket push — not REST): raw WebSocket at `ws://…/ws/notifications?token=<accessJWT>` (URL derived from `environment.apiUrl` with `http`→`ws`; the browser WebSocket API can't set headers, so the access token rides the query string and a backend handshake interceptor validates it). Server broadcasts one JSON text frame to every connected client every 30 min (`app.notification.interval-ms`): `Notification {message: string, timestamp: string}` — `timestamp` is ISO-8601 UTC, validated client-side with `Date.parse` before storing. Handshake auth is one-time; on reconnect the service refreshes first if the 15-min token expired.
