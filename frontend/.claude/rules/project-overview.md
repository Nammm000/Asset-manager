...
- The Spring Boot 3.2.3 / Java 17 backend lives in `../backend` (runs on port 8082, CORS allows this app's dev origin). Its detailed docs are in `../backend/CLAUDE.md` and `../backend/.claude/rules/` — notably `api-surface.md` (REST contract), `security-auth.md` (JWT), `asset-business-rules.md`, and `database-schema.md`.
